...
const CreateInvoiceModal = ({ isOpen, onClose, onInvoiceCreated }) => {
  ...
  const calculatedTaxAmount = (subtotal - Number(discountAmount)) * (Number(taxPercent) / 100);
  const total = subtotal - Number(discountAmount) + calculatedTaxAmount;
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2.5 outline-none focus:border-brand-teal focus:ring-1 focus:ring-brand-teal transition-all text-sm text-slate-900"
                  >
              ...
              <div>
                    ...
                    <div key={index} className="flex gap-3 items-end">
                      <div className="flex-1">
                        ...
                        <input
                          ...
                          className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-900 focus:border-brand-teal focus:ring-1 focus:ring-brand-teal"
                        />
                      </div>
                      <div className="w-24">
                ...
                <button
                  type="button"
                  onClick={addItem}
                  className="mt-4 text-sm text-brand-teal hover:text-teal-600 font-medium flex items-center"
                >
              ...
              <div className="border-t border-slate-200 pt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full h-32 bg-slate-50 border border-slate-200 rounded-lg px-4 py-2.5 outline-none focus:border-brand-teal focus:ring-1 focus:ring-brand-teal transition-all text-sm text-slate-900 resize-none"
                ...
                <div className="bg-slate-50 p-6 rounded-xl space-y-3 border border-slate-200">
                  ...
                  <div className="flex justify-between items-center text-sm gap-4">
                    ...
                    <input
                      ...
                      className="w-24 bg-white border border-slate-200 rounded px-2 py-1 text-right outline-none focus:border-brand-teal focus:ring-1 focus:ring-brand-teal text-slate-900"
                    />
                  </div>
                  <div className="flex justify-between items-center text-sm gap-4">
                    <span className="text-slate-600">Tax (%)</span>
                    <input 
                      type="number" min="0" step="0.1" value={taxPercent} onChange={(e) => setTaxPercent(e.target.value)}
                      className="w-24 bg-white border border-slate-200 rounded px-2 py-1 text-right outline-none focus:border-brand-teal focus:ring-1 focus:ring-brand-teal text-slate-900"
                    />
                  </div>
